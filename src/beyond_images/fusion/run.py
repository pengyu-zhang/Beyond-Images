...
Input: one or more entity summary JSON files produced by the captioning
merge step (typically the original-image and new-image files).
Output: the same entity schema with
...
def clean_descriptions(descriptions: list[str]) -> list[str]:
    """De-duplicate and drop degenerate captions."""
    cleaned = []
...
def collect_descriptions(
    ...
    (by default the original-image captions take priority).
    """
