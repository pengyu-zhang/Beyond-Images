...
The paper evaluates three image-to-text models (Table 5). All three are
implemented behind one interface, selected via `captioning.backend` in the
config.
...
Notes:
  ...
  - GIT is a pure captioning model and does not condition on a text prompt;
    the configured prompt is ignored for that backend.
"""
