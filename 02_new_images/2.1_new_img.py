...
def extract_table_data(soup: BeautifulSoup, base_id: str, page_url: str) -> Dict[str, str]:
    ...
    for table in tables:
        for row in table.find_all('tr'):
            ...
            value_cell = row.find('td', {'class': 'fileinfotpl-value'}) or row.find_all('td')[1] if len(row.find_all('td')) > 1 else row.find('td')
    ...
    if full_image_link_div:
        ...
        if main_image_tag and 'src' in main_image_tag.attrs:
            ...
            if src.startswith('//'):
                src = 'https:' + src
            return src
    log_error(f"No fullImageLink div found for {base_id} {page_url}")
    return None
...
def process_url_pair(url_pair: Tuple[str, str], save_path: str, intermediate_data: List[Dict[str, Any]], failed_images_log_file: str):
    """
    处理单个 URL 对，提取图片和表格信息并保存
    """
    base_url, base_id = url_pair
    qid = base_url.split('/')[-1]
    if not qid:
        log_error(f"Failed to extract QID for {base_url}")
        return
    ...
    for page_index, page_url in enumerate(image_pages):
        ...
        if image_url:
            ...
            download_image(image_url, save_path, image_name)
        ...
        entry = {
            ...
            "page_url": page_url,
            "image_url": image_url,
            "summary": table_data  # 保存完整表格内容
        }
        intermediate_data.append(entry)

def save_table_data(json_output_path: str, all_data: List[Dict[str, Any]]):
    """
    保存数据到 JSON 文件，按 ID 排序
    """
    try:
        all_data.sort(key=lambda entry: (int(entry['id'].split('_')[0]), int(entry['id'].split('_')[1])))
...
def main(input_file: str, save_path: str, json_output_path: str, start_index: int, end_index: int, failed_images_log_file: str):
    ...
    主函数，协调各模块功能
    """
    url_id_pairs = read_input_file(input_file)
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(process_url_pair, pair, save_path, intermediate_data, failed_images_log_file) for pair in selected_pairs]
...
if __name__ == "__main__":
    # 路径配置
    input_file = "/gpfs/home5/pzhang/beyond_images/datasets/log/MKG-W_entity2id.txt"
    save_path = "/gpfs/home5/pzhang/beyond_images/datasets/MKG-W_img_new"
    json_output_path = "/gpfs/home5/pzhang/beyond_images/datasets/MKG-W_img_new_summary.json"
    ERROR_LOG_FILE = "/gpfs/home5/pzhang/beyond_images/datasets/log/MKG-W_img_new_error_log.txt"
    FAILED_IMAGES_LOG_FILE = "/gpfs/home5/pzhang/beyond_images/datasets/log//MKG-W_img_new_failed_images_log.txt"
