...
Local-first Streamlit interface for auditing whether LLM-fused entity
summaries match the entity's image set (paper Section 4.4). See
tools/checker/README.md for data format and usage.
...
def get_description(record: Dict) -> str:
    """Fused summary if present, else the merge-stage concatenated captions."""
    return safe_get(record, "images.images_t5_descriptions", "") or str(
        safe_get(record, "images.merged_descriptions", "") or ""
    )
